...
open class DropDown : UITextField{
    ...
    didSet{
      if searchText == "" {
        ...
        self.dataArray = optionArray.filter {
          return $0.string.range(of: searchText, options: .caseInsensitive) != nil
        }
      }
      reSizeTable()
      selectedIndex = nil
  ...
  public func showList(_ tableBackgroundColor: UIColor? = nil) {
    ...
    if let color = tableBackgroundColor {
  ...
  public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if let selected = optionArray.firstIndex(where: {$0 == selectedText}) {
      if let id = optionIds?[selected] {
        didSelectCompletion(selectedText.string, selected , id )
      }else{
        didSelectCompletion(selectedText.string, selected , 0)
      }
